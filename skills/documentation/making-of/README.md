...
- **Show the code, not just the command.** A test-runner line by itself proves nothing to a reader who hasn't seen the class or annotation it's about and the test method that produced it — show both before the output, in the goal and the proof alike, and show the actual fix snippet rather than describing it.
...
- **One commit per point, titled like its section.** When the work ships as a branch, `git log --oneline` should read as the entry's table of contents, every intermediate commit green for its own tests, so a commit's diff can be read next to the section that explains it. Exceptions (two points that must land together, a section with no commit) are named in the section, not hidden.
- **Open each section with the files to open.** An italic line naming the commit and linking the two to five files to look at, then prose that points at the method or lines, not just the file — the reader has the code on the other half of the screen.
